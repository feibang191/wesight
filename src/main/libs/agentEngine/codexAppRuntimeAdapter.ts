...
export class CodexAppRuntimeAdapter extends EventEmitter implements CoworkRuntime {
  ...
  private ensureCodexWorkspace(cwd: string): void {
    try {
      ...
      fs.writeFileSync(
        memPath,
        '# Memory\n\n' +
          '> This workspace is managed by WeSight. Codex CLI is running here in ' +
          'managed mode. Edit this file to keep user-visible state across sessions.\n\n',
        'utf8',
      );
    } catch {
      // Bootstrap is best-effort; if the directory is not writable the CLI
      // will still surface its own prompt and the user can recover.
    }
  }
  ...
  private async runTurn(
    ...
    // Ensure a minimal MEMORY.md exists in the workspace. Codex CLI refuses
    // to do real work in a fresh workspace and instead prompts the user to
    // bootstrap. Writing a stub keeps the runtime usable on first launch.
    this.ensureCodexWorkspace(cwd);
