...
import { RuntimeCoordinator } from './libs/runtimeCoordinator';
import {
...
app.on('ready', () => {
  ...
  // Warm-start the heaviest engines in the background. The first time a
  // user picks Hermes or OpenClaw from the picker, the gateway spawn
  // dominates the visible "engine switch is sluggish" window. Kicking them
  // off here (without awaiting) lets the spawn run while the renderer is
  // still painting the main window.
  setImmediate(() => {
    void getHermesEngineManager()
      .startGateway()
      .catch((error: unknown) => {
        console.warn('[startup] Hermes warm-start failed:', error);
      });
    void getOpenClawEngineManager()
      .startGateway()
      .catch((error: unknown) => {
        console.warn('[startup] OpenClaw warm-start failed:', error);
      });
  });
});
...
const getRuntimeCoordinator = (): RuntimeCoordinator => {
  if (!runtimeCoordinator) {
    runtimeCoordinator = new RuntimeCoordinator({
      ...
      cliStatuses: () => getMergedExternalAgentEnvironmentSnapshot().engines,
    });
  }
...
const getCoworkStore = () => {
  if (!coworkStore) {
    ...
    const repairedCoreConfig = coworkStore.repairMissingCoreConfig();
    if (repairedCoreConfig.workingDirectory || repairedCoreConfig.executionMode || repairedCoreConfig.agentEngine) {
      console.log('[CoworkStore] repaired missing core cowork config:', repairedCoreConfig);
    }
    const repairedConfigSources = coworkStore.repairManagedExternalAgentConfigSources();
    if (repairedConfigSources.openclawConfigSource || repairedConfigSources.hermesConfigSource) {
      console.log('[CoworkStore] repaired persisted external agent config sources:', repairedConfigSources);
    }
...
const getCoworkFileActivityTracker = (): CoworkFileActivityTracker => {
  if (!coworkFileActivityTracker) {
    coworkFileActivityTracker = new CoworkFileActivityTracker((activity) => {
      ...
      const session = getCoworkStore().getSession(activity.sessionId);
      if (session?.workspaceId) {
        const workspaceActivity = getCoworkStore().recordWorkspaceActivity({
          workspaceId: session.workspaceId,
          kind: 'file',
          sessionId: activity.sessionId,
          agentId: session.agentId,
          title: path.basename(activity.filePath),
          detail: `${activity.status}: ${activity.filePath}`,
          payload: {
        ...
        windows.forEach((win) => {
          if (win.isDestroyed()) return;
          try {
            win.webContents.send(CoworkIpcChannel.StreamWorkspaceActivity, workspaceActivity);
          } catch (error) {
            console.error('[CoworkWorkspace] failed to forward workspace file activity:', error);
          }
        });
      }
...
const broadcastWorkspaceActivityForSession = (
  sessionId: string,
  input: {
    kind: 'session' | 'browser' | 'context' | 'delegation';
    title: string;
    detail: string;
  ...
  const activity = getCoworkStore().recordWorkspaceActivity({
    workspaceId: session.workspaceId,
    kind: input.kind,
    sessionId,
    agentId: session.agentId,
  ...
  BrowserWindow.getAllWindows().forEach((win) => {
    if (win.isDestroyed()) return;
    try {
      win.webContents.send(CoworkIpcChannel.StreamWorkspaceActivity, activity);
    } catch (error) {
      console.error('[CoworkWorkspace] failed to broadcast workspace activity:', error);
    }
  });
};
...
const bindCoworkRuntimeForwarder = (): void => {
  ...
  runtime.on('message', (sessionId: string, message: CoworkMessage) => {
    ...
    if (message.type === 'user' || message.type === 'assistant' || message.type === 'system') {
      broadcastWorkspaceActivityForSession(sessionId, {
        kind: 'session',
        title: message.type,
        detail: truncateIpcString(message.content, 220),
        payload: {
    ...
    if (message.metadata?.kind === 'workspace_browser_activity') {
      broadcastWorkspaceActivityForSession(sessionId, {
        kind: 'browser',
        title: 'browser',
        detail: truncateIpcString(message.content, 220),
    ...
    if (message.metadata?.kind === 'team_turn' || message.metadata?.kind === 'team_final') {
      broadcastWorkspaceActivityForSession(sessionId, {
        kind: 'delegation',
        title: String(message.metadata?.teamName || 'team'),
        detail: truncateIpcString(message.content, 220),
        payload: message.metadata as Record<string, unknown>,
        createdAt: message.timestamp,
      });
  ...
  runtime.on('complete', (sessionId: string, claudeSessionId: string | null) => {
    ...
    broadcastWorkspaceActivityForSession(sessionId, {
      kind: 'session',
      title: 'complete',
      detail: claudeSessionId ? `Session completed: ${claudeSessionId}` : 'Session completed',
    });
    const windows = BrowserWindow.getAllWindows();
    windows.forEach((win) => {
      if (win.isDestroyed()) return;
  ...
  runtime.on('error', (sessionId: string, error: string) => {
    ...
    broadcastWorkspaceActivityForSession(sessionId, {
      kind: 'session',
      title: 'error',
      detail: truncateIpcString(error, 220),
      payload: { error },
    });
...
const broadcastCoworkMessage = (sessionId: string, message: CoworkMessage): void => {
  if (message.type === 'user' || message.type === 'assistant' || message.type === 'system') {
    broadcastWorkspaceActivityForSession(sessionId, {
      kind: message.metadata?.kind === 'team_turn' || message.metadata?.kind === 'team_final' ? 'delegation' : 'session',
      title: String(message.metadata?.teamName || message.type),
      detail: truncateIpcString(message.content, 220),
      payload: message.metadata as Record<string, unknown> | undefined,
      createdAt: message.timestamp,
    });
  }
  const safeMessage = sanitizeCoworkMessageForIpc(message);
...
<html lang="zh-CN">
  ...
  ipcMain.handle(CoworkIpcChannel.RuntimeStatusList, async () => {
    try {
      return {
        success: true,
        statuses: getRuntimeCoordinator().listStatuses(),
      };
    } catch (error) {
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to list unified runtime statuses',
      };
    }
  });

  ipcMain.handle(CoworkIpcChannel.WorkspaceState, async (_event, input: { workspaceId?: string; sessionId?: string; cwd?: string }) => {
    try {
      const session = input?.sessionId ? getCoworkStore().getSession(input.sessionId) : null;
      const workspaceId = (input?.workspaceId || session?.workspaceId || (input?.cwd ? getCoworkStore().getWorkspaceIdForCwd(input.cwd) : '')).trim();
      if (!workspaceId) {
        return { success: false, error: 'workspaceId is required' };
      }
      return {
        success: true,
        state: getCoworkStore().getWorkspaceSharedState(workspaceId),
      };
    } catch (error) {
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to load workspace shared state',
      };
    }
  });
  ...
  ipcMain.handle(CoworkIpcChannel.AgentEnginesList, async () => {
    try {
      return {
        success: true,
  ...
  ipcMain.handle(CoworkIpcChannel.AgentCustomPathUpdate, async (_event, input: {
    appType?: unknown;
    field?: unknown;
    value?: unknown;
  }) => {
    try {
      ...
      if (input?.field !== 'commandPath' && input?.field !== 'configPath') {
        return { success: false, error: 'Invalid custom path field.' };
      }
      if (typeof input?.value !== 'string') {
        return { success: false, error: 'Invalid custom path value.' };
      }

      updateExternalAgentCustomPath(input.appType, input.field, input.value);
      return {
        success: true,
        snapshot: getMergedExternalAgentEnvironmentSnapshot(),
      };
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to update custom agent path',
      };
    }
  });
  ...
  const initApp = async () => {
    ...
    const startupCoworkConfig = getCoworkStore().getConfig();
    const shouldAutoStartOpenClaw = isOpenClawCoworkAgentEngine(resolveCoworkAgentEngine())
      || startupCoworkConfig.openclawConfigSource === ExternalAgentConfigSource.WesightModel;
    const shouldAutoStartHermes = resolveCoworkAgentEngine() === CoworkAgentEngineValue.Hermes
      || startupCoworkConfig.hermesConfigSource === ExternalAgentConfigSource.WesightModel;

    if (shouldAutoStartOpenClaw) {
    ...
    if (shouldAutoStartHermes) {
      void ensureHermesRunningForCowork().then((status) => {
        if (status.phase === 'running') {
          startHermesIMSessionSyncPolling();
          void syncHermesIMSessionsToCowork('startup');
        }
      }).catch((error) => {
        console.error('[Hermes] Failed to auto-start gateway on app startup:', error);
      });
    }
    ...
    // Reconnect managed runtimes after system wake from sleep/suspend.
    powerMonitor.on('resume', () => {
      if (openClawRuntimeAdapter) {
        openClawRuntimeAdapter.onSystemResume();
      }
      void getHermesEngineManager().ensureReady().then((status) => {
        if (status.phase === 'running') {
          return;
        }
        return getHermesEngineManager().restartGateway();
      }).catch((error) => {
        console.warn('[Hermes] Failed to recover gateway after system resume:', error);
      });
    });
